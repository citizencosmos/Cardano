...
echo "You entered a Tax rate of: " awk -vn=$TAX_RATIO BEGIN{print(($TAX_RATIO)*100)"%"}
...
echo "shown in lovelaces" awk -vn=$TAX_FIXED BEGIN{print(($TAX_FIXED)*1000000)"Lovelaces"}
#TODO: error check confirmation, for now, just do it
...
echo "shown in lovelaces" awk -vn=$TAX_LIMIT BEGIN{print(($TAX_LIMIT)*1000000)"Lovelaces"}
#TODO: error check confirmation, for now, just do it
